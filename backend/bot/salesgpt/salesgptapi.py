...
class SalesGPTAPI:
    ...
    def __init__(
        self, config_path: str, verbose: bool = False, max_num_turns: int = 50
    ):
    ...
    def do(self, conversation_history: [str], human_input=None):
        if self.config_path == "":
            ...
            if self.USE_TOOLS:
                sales_agent = SalesGPT.from_llm(
                    self.llm,
                    use_tools=True,
                    product_catalog="businessData/sample_product_catalog.txt",
                    salesperson_name="Botty",
        ...
        return reply.split(f"{sales_agent.salesperson_name}: ")
